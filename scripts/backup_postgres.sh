...
BACKUP_DIR="${BACKUP_DIR:-/var/backups/zapfluxo}"
RETENTION_DAYS="${BACKUP_RETENTION_DAYS:-14}"
timestamp="$(date -u +%Y%m%dT%H%M%SZ)"
destination="$BACKUP_DIR/zapfluxo-$timestamp.dump"
...
find "$BACKUP_DIR" -type f -name 'zapfluxo-*.dump' -mtime "+$RETENTION_DAYS" -delete
